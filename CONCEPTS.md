...
### Stage cache
Opt-in local content-hash cache (`src/stageCache.ts`) for the curate, script, and earEdit stages. When `STAGE_CACHE_DIR` is set, each stage's JSON-serializable output is stored under `$STAGE_CACHE_DIR/<stage>-<sha256(input)[:32]>.json` so a local re-run after a late TTS/audio/publish failure reuses the LLM results instead of re-paying for them. The script key includes `recentStyle` snippets and the phrase profile; the earEdit key includes intro/segments/outro plus per-cluster notes. Cache read/write errors are non-fatal. Unset disables caching entirely. Single-machine only — the daily Actions runner is ephemeral, so this never applies in CI. Distinct from Already-published skip (which skips the whole paid pipeline when today's episode is already on disk).
...
### Host identity
The single persistent host's identity — background, beat, what they care about, how they talk, their humor, and what they refuse to do (hype, hedge everything equally, perform surprise, moralize, play the accent for laughs) — defined once in `src/voice.ts` (`HOST_IDENTITY`) and rendered into every script prompt via `formatHostIdentityBlock()`. The host is Southern by upbringing: the dialect lives in the words (standard spelling, no recurring sayings), and the accent is supplied separately by the TTS delivery instructions in `src/speakerProfiles.ts`. Replaced the five rotating `DAILY_PERSONAS`; the tonal lens is now constant across episodes instead of selected by date. Production prefers Show config over these defaults.

### Show config
The listener-edited voice and tone in `config/show.json`, loaded by `src/showConfig.ts`. It carries the host identity, register exemplars, free-text tone notes, and spoken-delivery instructions. Blank fields fall back to the built-ins in `src/voice.ts` and `src/speakerProfiles.ts`. The tune page at `docs/tune/` commits the file straight to `main`, so the next morning's run picks it up without a pull request. A non-empty `TTS_VOICE` or `TTS_*_STYLE` environment variable still wins over the file's delivery fields. Tone notes are appended at the end of the script and ear-edit prompts and outrank the default register, including the Emphasis budget, when they conflict. They do not outrank facts.

### Voice exemplars
Three to five passages in the host's register (`VOICE_EXEMPLARS` in `src/voice.ts`), quoted into the script prompt as "the show at its best." Currently hand-written seeds in the Southern register; replace with real passages from published transcripts as episodes in this voice land. The model is told to match their register — the flatness, the specificity, the way a judgment lands without being announced — never their wording. Curated by hand; replace with better passages as new episodes publish.

### Emphasis budget
The prompt's positive replacement for a list of banned rhetorical moves: baseline register stays flat, declarative, and specific; the script gets one deliberate rhetorical peak at its most consequential story, at most one analogy (and only if it maps to something the listener has lived), and no two consecutive sentences sharing the same rhetorical shape. Spends rhetoric like a budget instead of forbidding it outright. Listener tone notes in Show config outrank this budget when they conflict.
...
### Ear edit
A low-temperature copy-editing pass (`src/earEdit.ts`) that runs between script and TTS: the same script comes back as JSON, with only the Emphasis budget mechanically enforced — warm-up sentences and self-endorsements deleted, runs of same-shape sentences broken up, unearned triads collapsed. Non-blocking: any failure (bad JSON, a validator trip, a word-count blowout) falls back to the unedited script, so this stage can only leave an episode equal to or better than what the script stage produced. Toggle with `EAR_EDIT_ENABLED` (default on). Included in the Stage cache when that is enabled.

### Section cues
Short intro / transition / outro stingers mixed into the program master by `src/audio.ts`. Controlled by `AUDIO_CUES_ENABLED` (default on) and `AUDIO_CUE_STYLE` (`tone`, `chime`, `tick`, or `asset`). Each cue carries ~0.7s of silence padding at section boundaries (intro: after only; transition: both sides; outro: before only) so the show breathes between stories. `asset` mode loads `assets/audio/cue-{intro,transition,outro}.{mp3,wav}` and falls back to `tone` if any file is missing. Cue durations are folded into chapter timestamps. Distinct from Inline audio tags and Delivery hint (those are TTS speech channels, not program structure).

### Delivery hint
An optional 3-6 word per-segment spoken-delivery note (e.g. "flat — let the number speak") that the script writer attaches to a segment. `src/tts.ts` folds it into that segment's TTS instructions on the OpenAI path, and into `speech_metadata.style` on the direct Gemini path. The OpenRouter path has no delivery-instructions channel and uses inline audio tags instead. Transient: carried from script to tts, not persisted to the sidecar.
