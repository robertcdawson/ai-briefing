...
OpenAI does not label built-in voices by gender in the API docs, but the current Speech API includes `alloy`, `ash`, `ballad`, `coral`, `echo`, `fable`, `nova`, `onyx`, `sage`, `shimmer`, `verse`, `marin`, and `cedar`. In practice, `cedar` (the default, warm and grounded) and `marin` are OpenAI's recommended best quality; `sage` and `verse` are good natural-sounding alternates; `coral`, `nova`, or `shimmer` read brighter/feminine-coded.
